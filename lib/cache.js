...
Cache.prototype.filename = function(url) {
  var cacheName;
  // generate a new file name
  do {
    // this.path + '/' + Math.random().toString(36).substring(2)
    var filePath = this.path + '/' + url.replace(/^https?:\/\//, '');
    if (!filePath.match(/\.tgz$/)) {
        filePath += '/package.json';
    }
    cacheName = path.normalize(filePath);
  } while (fs.existsSync(cacheName));

  mkdirp.sync(path.dirname(cacheName));
